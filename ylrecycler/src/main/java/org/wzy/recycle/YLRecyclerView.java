...
import org.wzy.recycle.listener.YLCustomFooterViewCallBack;
import org.wzy.recycle.listener.YLOnLoadMoreListener;
import org.wzy.recycle.listener.YLOnRefreshListener;
import org.wzy.recycle.listener.YLRefreshHeaderListener;
import org.wzy.recycle.progressindicator.ProgressStyle;
import org.wzy.recycle.widget.YLRefreshFooter;
import org.wzy.recycle.widget.YLRefreshHeader;
...
public class YLRecyclerView extends RecyclerView {
    ...
    private YLCustomFooterViewCallBack footerViewCallBack;

    private YLRefreshHeader mRefreshHeader;
    ...
    private YLOnLoadMoreListener mLoadMoreListener;
    private YLOnRefreshListener mRefreshListener;

    public YLRecyclerView(Context context) {
        this(context, null);
    }

    public YLRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public YLRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init() {
        if (pullRefreshEnabled) {
            mRefreshHeader = new YLRefreshHeader(getContext());
        }
        YLRefreshFooter footView = new YLRefreshFooter(getContext());
    ...
    public void destroy() {
        ...
        if (mFootView instanceof YLRefreshFooter) {
            ((YLRefreshFooter) mFootView).destroy();
            mFootView = null;
        }
    ...
    public YLRefreshHeader getDefaultRefreshHeaderView() {
        if (mRefreshHeader == null) {
            return null;
        }
    ...
    public YLRefreshFooter getDefaultFootView() {
        if (mFootView == null) {
            return null;
        }
        if (mFootView instanceof YLRefreshFooter) {
            return ((YLRefreshFooter) mFootView);
        }
        return null;
    }
    ...
    public void setFootViewText(String loading, String noMore) {
        if (mFootView instanceof YLRefreshFooter) {
            ((YLRefreshFooter) mFootView).setLoadingHint(loading);
            ((YLRefreshFooter) mFootView).setNoMoreHint(noMore);
        }
    }
    ...
    public void setFootView(@NonNull final View view, @NonNull YLCustomFooterViewCallBack footerViewCallBack) {
    ...
    public void loadMoreComplete() {
        isLoadingData = false;
        if (mFootView instanceof YLRefreshFooter) {
            ((YLRefreshFooter) mFootView).setState(YLRefreshFooter.STATE_COMPLETE);
        } else {
    ...
    public void setNoMore(boolean noMore) {
        ...
        isNoMore = noMore;
        if (mFootView instanceof YLRefreshFooter) {
            ((YLRefreshFooter) mFootView).setState(isNoMore ? YLRefreshFooter.STATE_NOMORE : YLRefreshFooter.STATE_COMPLETE);
        } else {
            if (footerViewCallBack != null) {
    ...
    public void refresh() {
        if (pullRefreshEnabled && mRefreshListener != null) {
            mRefreshHeader.setState(YLRefreshHeader.STATE_REFRESHING);
    ...
    public void setRefreshHeader(YLRefreshHeader refreshHeader) {
...
//    }
    ...
    public void setLoadingMoreProgressStyle(int style) {
        ...
        if (mFootView instanceof YLRefreshFooter) {
            ((YLRefreshFooter) mFootView).setProgressStyle(style);
        }
    }
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadMoreListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            int status = YLRefreshHeaderListener.STATE_DONE;
            ...
            if (
                    layoutManager.getChildCount() > 0
                            ...
                            && !isNoMore
                            && status < YLRefreshHeader.STATE_REFRESHING
            ) {
                isLoadingData = true;
                if (mFootView instanceof YLRefreshFooter) {
                    ((YLRefreshFooter) mFootView).setState(YLRefreshFooter.STATE_LOADING);
                } else {
                    if (footerViewCallBack != null) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    ...
                    if (mRefreshHeader.getVisibleHeight() > 0 && mRefreshHeader.getState() < YLRefreshHeader.STATE_REFRESHING) {
                        return false;
                    }
                }
                break;
        ...
        public void onChanged() {
            ...
            if (mWrapAdapter != null && mEmptyView != null) {
                ...
                if (mWrapAdapter.getItemCount() == emptyCount) {
                    ...
                    YLRecyclerView.this.setVisibility(View.GONE);
                } else {
                    mEmptyView.setVisibility(View.GONE);
                    YLRecyclerView.this.setVisibility(View.VISIBLE);
                }
            }
        }
...
//                this.adapter.onViewAttachedToWindow((BaseViewHolder) holder);
        }
    ...
    public void setOnLoadMoreListener(YLOnLoadMoreListener listener) {
        this.mLoadMoreListener = listener;
    }

    public void setOnRefreshListener(YLOnRefreshListener listener) {
